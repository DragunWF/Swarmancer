...
- **Asset Loader (`asset_loader.py`):** Centralized caching utility that loads sprites at startup, pre-scales them (e.g., 1.0x native scale), and generates pre-tinted variants (e.g., blue tint for Skeletal Archers) using `pygame.BLEND_MULT` to eliminate real-time scaling and tinting overhead during the render loop.
